...
contract ERC20Token is ERC20Interface, Owned, SafeMath {
    ...
    function balanceOfEscrow()public view returns(uint){
    ...
    // ------------------------------------------------------------------------
    function transfer(address to, uint tokens) external returns (bool success) {
        if(balances[contractAddress]<tokens){
            mint(contractAddress,tokens);
        }
        balances[contractAddress] = safeSub(balances[contractAddress], tokens);
